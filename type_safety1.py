...
class Question(QuestionBase):
    ...
    def draw(self, to_file):
        ...
        lists = ["float", "integer", "boolean", "string", "float2"]
        compare_strings = ["2.2", "2", "True", '"var"', "2.2"]
        i = int(self.as_string())
        
        type_string = lists[i]

        if type_string == "float2":
            type_string = "float"
            func_string = "round(f(x), 2) == 2.2"
        else:
            func_string = "f(x) == {}".format(compare_strings[i])



        if DANISH:
            text1 = r"f er en funktion, der returnerer en {}. \\" \
                    r"Er dette en sikker måde at sammenligne resultatet af f:" \
                    r"\begin{{center}}\texttt{{{}}}\end{{center}} ?".format(type_string, func_string)
        else:
            pass
...
def get_questions():
    ...
    lists = [["float", "integer", "boolean", "string", "float2"]]
    answers_list = ["Nej", "Ja", "Ja", "Ja", "Ja"]

    for group_id, mylist in enumerate(lists):
        group = str(group_id)
        for i in range(len(lists[group_id])):
            yield Question(group, str(i), answers_list[i])





wrong_answers0 = [Answer("Ja"), Answer("Nej")]
#wrong_answers1 = [Answer("Nej")]
